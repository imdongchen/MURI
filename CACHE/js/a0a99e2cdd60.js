...
function updateMap() {
    ...
    dFootprint.top(Infinity).forEach(function(p, i) {
...
function onSelectFeature(feature) {
    ...
    dFootprint.filterFunction(function(d) {
        d.footprints.forEach(function(fp) {
            for (var i = 0; i < selectedFeas.length; i++) {
                if (fp.id == selectedFeas[i]) return true;
            }
        });
        return false;
    });
