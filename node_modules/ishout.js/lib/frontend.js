...
function createRealtimeServer(storage, sslOptions, socketOptions) {
  ...
  if (socketOptions)
    socketSettings = utils.mergeOptions(socketSettings, socketOptions);

  var io = socketio.listen(server, socketSettings); 

  var auth = new api.AuthorizationHandler(storage);

  function setupConnection(socket) {
    socket.on('ishout-authentication', function(authData) {
      // validate token and authorize user
      auth.authenticate(authData.authString, function(userId){
        if (!userId) {
          socket.emit('ishout-authentication-response', {
            status : 'failed'
          });
          return;
        }
        ...
        // handle rooms
        socket.on('ishout-room-join', function(roomData) {
